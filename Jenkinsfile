    ...
    stages {
        stage('Clone Repository') {
            steps {
                ...
                script {
                    // Use --pretty with correct format
                    def commitMessage = bat(script: 'git log -1 --pretty=%s%n%b', returnStdout: true).trim()
                    echo "Commit message: ${commitMessage}" // Debugging output
